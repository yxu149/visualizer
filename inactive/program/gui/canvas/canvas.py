...
from gui.panels import Button_Bar
from gui.utils import cursor, CursorState
...
class Sketchpad(tk.Frame):
    ...
    def __init__(self,root,canvas_width=2000,canvas_height=1300,event_root=None,onselect=None,onrelease=None):
        ...
        self.ver_scrollbar = tk.Scrollbar(self, orient=tk.VERTICAL)
        ...
        self.canvas.grid(row=0,column=0,sticky=('nswe')) # places the canvas in row : 0 , column :0 in the tk.Frame
        self.rowconfigure(0,weight=1)
        self.columnconfigure(0,weight=1)
        self.hor_scrollbar.grid(column=0, row=1, sticky=('we'))
        self.ver_scrollbar.grid(column=1, row=0, sticky=('ns'))
        self.__on_select = onselect
        self.__on_release = onrelease
        cursor.set_callback(self.undo_selection) # add callback function when cursor changes its state (event)
    ...
    def undo_selection(self):
        ...
        '''
        function called when CursorState changes to reset the selections to its initial state (Nothing is selected)
        '''
    ...
    def mouse_clicked(self,event):
        ...
        '''
        function for listening to cursor click event on canvas and if the CursorState was circle try to place circles in clicked place
        '''

        if cursor.get_state() == CursorState.circle: # check for cursor state
    ...
    def node_clicked(self,event,canvas_item_id):
        ...
        '''
        function for listening to cursor click event on node it has many cases :
        1st case: cursor state was line and self.connection_node is not null so its the second point so we should connect the two points
        2nd case: cursor state was line and self.connection_node is null so we assign it to self.connection_node and wait for second node in order to connect them
        3rd case: cursor state is normal (neither circle nor line are selected) and self.selected is None so no node is selected so we self.selected will be equal to clicked node
        4th case: cursor state is normal and self.selected is not None so the we will deselect old node and select the new one
        5th case: cursor state is normal and self.selected is equal to clicked node so we will deselect this node and makes self.selected = None 
        '''

        if cursor.get_state() == CursorState.line: # checking for cursor state
            # 2nd case
        ...
        elif cursor.get_state() == CursorState.normal:
            # 3rd case
        ...
        elif cursor.get_state() == CursorState.initial_node:
        ...
        elif cursor.get_state() == CursorState.goal_node:
    ...
    def line_clicked(self,event,canvas_item_id):
        ...
        if cursor.get_state() == CursorState.normal:
